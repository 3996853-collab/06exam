...
【提取原则与业务规则】
1. 单号 (delivery_no):
   - 【极其重要的第一优先级】：重点识别位于图片【右下角】的【二维码 (QR Code)】！解析二维码中包含的单号文本或读取右下角二维码旁紧邻标注的编码串；
   - 【第二优先级】：若右下角二维码缺失或彻底模糊，则寻找图片上的“出库单号”、“送货单号”、“单号”、“凭证编号”或顶部条形码；
   - 提取时仅保留连续英数字符（如 CK202609040001、ZTO8899123 等），彻底剔除中文文字（如“出库单号：”）、空格及特殊标点符号；
   - 若右下二维码及单号字迹严重模糊无法辨认或确实缺失，返回 null，严禁猜测或伪造。

2. 签收日期 (sign_date) 与签收时间 (sign_time):
   - 【核心规则】：识别图片水印（如今日水印相机、马克水印相机、钉钉/企业微信打卡水印等）上的【日期】和【打卡时间/拍照时间】作为签收日期和签收时间；
   - 定位图片边缘、四角（通常位于底部、左下角或右下角水印层）生成的半透明或叠加数字水印，或相机水印模板中的“时间”、“打卡时间”、“签收时间”项；
            ...
            "messages": [
                ...
                },
                {
                    ...
                    "content": [
                        {
                            "type": "text",
                            "text": "请仔细分析这张回单照片：重点识别位于图片【右下角的二维码】提取出单号（若有旁边文字辅助参考请结合提取），并识别相机水印上的【日期】与【打卡时间/拍照时间】作为签收日期和时间。严格直接输出目标 JSON。"
                        },
                        {
                            "type": "image_url",
                            "image_url": {
